...
describe('calculateTax(総合・売上600万/経費100万/青色65万/国保/40歳未満)', () => {
  ...
  it('集計(税合計・負担合計・手取り・実効税率・毎月の3分解)', () => {
    ...
    // 毎月の3分解: ①固定費(社保/12) + ②納税つみたて(税/12) + ③手取り(/12)
    expect(r.monthlyFixedCost).toBe(53_487); // 社会保険 641,840 / 12 切り上げ
    expect(r.monthlyTaxReserve).toBe(55_650); // 税 667,800 / 12
    expect(r.monthlyTakeHome).toBe(307_530); // 手取り 3,690,360 / 12
  });
});
...
describe('calculateTax(低所得・扶養内・白色)', () => {
  ...
  it('手取りと毎月の3分解(扶養内=固定費0)', () => {
    ...
    expect(r.monthlyTaxReserve).toBe(4_859); // 税 58,300 / 12 切り上げ
    expect(r.monthlyTakeHome).toBe(61_808); // 手取り 741,700 / 12 切り捨て
  });
});
